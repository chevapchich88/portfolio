...
def calculate_demographic_data(print_data=True):
    ...
    # Count race are represented in this dataset.
    ...
    pr_per_country = df['native-country'].loc[df['salary'] == '>50K'].value_counts() / df['native-country'].value_counts() * 100
    highest_earning_country = pr_per_country.idxmax()
    highest_earning_country_percentage = round(pr_per_country.max(), 1)

    # Identify the most popular occupation for those who earn >50K in India.
    top_IN_occupation = df['occupation'].loc[(df['salary'] == '>50K') & (df['native-country'] == 'India')].value_counts().index[0]
